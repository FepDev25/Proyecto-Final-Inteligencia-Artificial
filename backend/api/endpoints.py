...
MODEL_PATH = "./modelo/Mejor_Modelo.pth"
ETIQUETAS_PATH = "./modelo/clases_nuevas.txt"
IMAGES_DIR = "./images"
...
async def predict(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    temp_file_location = f"./temp_{unique_filename}"
    permanent_file_location = os.path.join(IMAGES_DIR, unique_filename)
    
    # Guardar archivo temporalmente para procesamiento
    with open(temp_file_location, "wb") as f:
        f.write(await file.read())

    diagnostico_nombre, prob, otras_condiciones, sintomas = predecir_imagen_completa(modelo, temp_file_location, etiquetas, db)

    # Mover la imagen al directorio permanente
    shutil.move(temp_file_location, permanent_file_location)
    ...
    db_prediccion = models.Prediccion(
        ...
        url_imagen=unique_filename,  # Guardar solo el nombre del archivo
    ...
    for condicion in otras_condiciones:
        db_posible_condicion = models.PosibleCondicion(
            ...
            nombre_interno=condicion['condicion'],  # Usamos el nombre completo también como interno
...
async def get_image(image_name: str):
    """Endpoint para servir imágenes de predicciones"""
...
def read_predicciones(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    predicciones = db.query(models.Prediccion).options(
        joinedload(models.Prediccion.enfermedad)
...
def read_prediccion(prediccion_id: int, db: Session = Depends(get_db)):
    db_prediccion = db.query(models.Prediccion).options(
        joinedload(models.Prediccion.enfermedad)
    ).filter(models.Prediccion.id_prediccion == prediccion_id).first()
